...
class MLMDataset(Dataset):
    ...
    def __init__(self, name, split, social_dim, data_dir):
        ...
        elif name == 'flowmetrics':
            self.times = list(data.time_idx.astype(int))
...
class SADataset(Dataset):
    ...
    def __init__(self, name, split, social_dim, data_dir):
        ...
        self.users = list(data.user)
        self.years = list(data.year)
        self.months = list(data.months) if 'months' in data else list(data.month)
        self.days = list(data.day)
...
def load_external_data(name, social_dim, data_dir):
    ...
    with open('{}/{}_edges.p'.format(data_dir, name), 'rb') as f:
        ...
        if name == 'reddit' or name == 'arxiv' or name == 'flowmetrics':  # flowmetrics: weighted edge triples -> pairs
            edge_set = set(e[:2] for e in edge_set if e[2] > 0.01)

    with open('{}/{}_users.p'.format(data_dir, name), 'rb') as f:
        users = pickle.load(f)

    if name == 'arxiv' or name == 'reddit' or name == 'flowmetrics':  # flowmetrics: undirected graph
        graph = nx.Graph()
    else:
        graph = nx.DiGraph()
    ...
    with open('{}/{}_vectors_{}.txt'.format(data_dir, name, social_dim), 'r') as f:
        ...
        for i, l in enumerate(f):
            ...
            elif name == 'arxiv' or name == 'reddit' or name == 'yelp' or name == 'flowmetrics':  # flowmetrics: string node names
